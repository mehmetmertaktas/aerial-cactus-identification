...
class Utilities:
    ...
    def display_data(self, data, n=10, classes=None):
        ...
        for i, j in enumerate(indices):
            ax[i].imshow(np.transpose((data[j][0] - low) / (high - low), 
                                      (1, 2, 0)))
            ax[i].axis('off')
            if classes:
    ...
    def train(self,
        ...
        for epoch in range(epochs):
            ...
            if valid_loader is not None:
                vl, va = self.validate(model, valid_loader, loss_fn=loss_fn)
    ...
    def plot_history(self, history, validation=False):
    ...
    def save_results(self, path, dataset, model):
        model.eval()
        result = []
        with torch.no_grad():
            for datapoint in dataset:
                X = datapoint[0][None, ...].to(self.device)
                out = model(X)
